...
neighbourhood_count = listings_df['小区'].value_counts()
small_area = neighbourhood_count.index.tolist()
source = neighbourhood_count.values.tolist()
print(small_area)
print(source)

# 创建表格数据
trace=go.Table(header=dict(values=['小区名','房源数量'],
                           line_color="black", # 表头线条颜色
                           fill_color="#44cef6",  # 表头填充色
                           align="center"),             # 文本居中
               cells=dict(values=[small_area,    # 第1列数据
                                  source],     # 第2列数据
